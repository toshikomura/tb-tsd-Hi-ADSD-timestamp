...
Calcula_Latencia( int token, int n){
    ...
    /* verifica mudan̤as de estado dos nodos (latencia) */
    for ( i = 0; i < n; i++){
...
int Checa_se_Terminou_o_Diagnostico( int n){
    ...
    for ( i = 0; i < n; i++ ){ // para cada nodo
        for ( j = 0; j < n; j++ ){ // para cada posi̤̣o do timestamp do nodo

            /* se i esta sem falha e no timestamp de i na posi̤̣o j ̩ diferente do estado de j */
...
Iniciando_Simulacao( int *termina_diagnostico_nodos, int n){
    ...
    /* defini̤̣o de tempo de erro e reparo */
...
//    schedule(repair, 70.0, 1);
//    schedule(repair, 70.0, 3);
//    schedule(repair, 70.0, 4);
...
Imprime_Log( node_set *nodes, int *num_cluster, int token, int max_cluster, int j, int n){
    ...
    /* checa se o nodo testado ṇo ̩ maior que a quantidade de nodos */
    if (nodes->nodes[j] < n) {
        ...
        if (nodes->nodes[j] < n){
            ...
            if (nodes_j->nodes[0] < n){
                ...
                if ( nodes_j->nodes[0] == token || ( nodo[token].timestamp[ nodes_j->nodes[0] ] % 2 ) == 1 ){
                    ...
                    /* se o primeiro nodo de C j,s ṇo for i, mas estiver falho */
                    if ( ( nodo[token].timestamp[ nodes_j->nodes[0] ] % 2 ) == 1 ){
                        ...
                        for(k = 1; k < nodo[token].cluster; k++){
                            ...
                            for(l = 0; l < nodes->size; l++){
                                // se ṇo for o token e se um desses nodos ṇo estiver falho
                    ...
                    if ( precisa_testar == 1){
                        ...
                        /* se primeira vez que seta o timestamp de i * posi̤̣o j */
...
Verifica_Diagnostico( int *inicia_diagnostico_nodos, int *termina_diagnostico_nodos, int *maior, int n){
    ...
    if (Checa_se_Iniciou_o_Diagnostico( inicia_diagnostico_nodos, termina_diagnostico_nodos, n)){
        if ( Checa_se_Terminou_o_Diagnostico( n ) ){
            ...
            for (i = 0; i < n; i++){ // para cada posi̤̣o do timestamp do vetor
                maior[i] = nodo[0].latencia[i];
                for (j = 1; j < n; j++){ // para cada nodo
                    /* se o nodo ṇo esta falho */
...
main ( int argc, char *argv[]) {
    ...
    *inicia_diagnostico_nodos, *termina_diagnostico_nodos, *maior;
    ...
    if (argc != 2) {
        puts ("N̼mero errado de parametros, uso correto, ./temp |n̼mero de nodos|");
        exit(1);
    }
    ...
    /* inicio da aloca̤̣o de ponteiros */
    ...
    /* final da aloca̤̣o de ponteiros */
    ...
    while (time() < 80.0) {
        ...
        case test:
            ...
            /* checa se esta no ̼ltimo cluster ou ṇo */
        ...
        case fault:
            ...
            if (r != 0) {
                printf ("Ṇo consegui falhar nodo %d\n", token);
                exit(1);
            }
            printf("++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
