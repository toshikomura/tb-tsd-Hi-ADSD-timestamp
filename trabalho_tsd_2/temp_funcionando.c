...
main ( int argc, char *argv[]) {
    ...
    event;
    int r, s,
    i, j, k, l, m, latencia_maior, latencia_testes,
    max_cluster, num_cluster = 0,
    terminou_diagnostico,
    precisa_testar,
    *maior, *inicia_diagnostico_nodos, *termina_diagnostico_nodos;
    char fa_name[5];
    node_set* nodes,* aux_nodes, *nodes_j, *nodes_k;
                ...
                if (nodes->nodes[j] < n){
                    ...
                    if ( nodes_j->nodes[0] == token || ( nodo[token].timestamp[ nodes_j->nodes[0] ] % 2 ) == 1 ){
                        ...
                        if ( ( nodo[token].timestamp[ nodes_j->nodes[0] ] % 2 ) == 1 ){
                            ...
                            for(k = 1; k < nodo[token].cluster; k++){ // para cada nodo pertencente a clusters anteriores
...
//                                printf("maior = %d < nodo[%d].latencia[%d] = %d\n", maior[i], j, i, nodo[j].latencia[i]);
                                if ( maior[i] < nodo[j].latencia[i]){
                                    maior[i] = nodo[j].latencia[i];
//                                    printf("atualizaou maior para %d\n", maior[i]);
                                }
